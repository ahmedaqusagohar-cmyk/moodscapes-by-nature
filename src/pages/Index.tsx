...
const Index = () => {
  ...
  // Apply mood to <html> + persist + SEO title
  useEffect(() => {
  ...
  }, [current.audio]); // eslint-disable-line
  ...
  return (
    <main className="mood-stage relative h-screen w-screen overflow-hidden">
      <MoodScene mood={mood} />
        ...
        <header className="fade-swap absolute left-0 right-0 top-0 z-20 flex items-center justify-between px-6 py-6 md:px-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs uppercase tracking-[0.25em] opacity-70">{current.tagline}</p>
            </div>
          </div>
          <Clock />
        </header>
      ...
      {!focus && (
        ...
        >
          <div className="text-7xl md:text-8xl mb-4 select-none" aria-hidden>
            {current.emoji}
          </div>
          <blockquote
            className="text-xl md:text-3xl leading-relaxed"
            style={{ fontFamily: "'Fraunces', serif", color: "hsl(var(--mood-ink))" }}
          >
            “{current.quote}”
          </blockquote>
          <div className="mt-4 text-xs uppercase tracking-[0.3em] opacity-60">— {current.name} mood</div>
        </section>
      )}
      ...
      {focus && (
        <button
          ...
          className="icon-btn absolute right-6 top-6 z-30"
      ...
      {!focus && (
        <div className="fade-swap absolute bottom-0 left-0 right-0 z-20 px-4 pb-6 md:px-10 md:pb-8">
          <div className="glass mx-auto flex max-w-5xl flex-col gap-4 rounded-3xl p-4 md:p-5">
            {/* Mood selector */}
            ...
            {/* Audio controls */}
            <div className="flex flex-wrap items-center gap-4 border-t pt-4" style={{ borderColor: "hsl(var(--mood-ink) / 0.12)" }}>
              <button
              ...
              <div className="hidden text-sm opacity-70 md:block">
                Now playing: <span className="font-medium">{current.name} ambience</span>
              </div>
